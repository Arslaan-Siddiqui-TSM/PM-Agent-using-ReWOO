...
from src.agents.document_classifier import DocumentClassifierAgent, DocumentClassification
from src.agents.content_extractor import ContentExtractorAgent, ExtractedContent
from src.core.document_analyzer import DocumentAnalyzer, DocumentAnalysisReport
from src.core.cache_manager import CacheManager
from src.config.document_intelligence_config import (
...
class DocumentIntelligencePipeline:
    ...
    def process_documents_with_rag(
        ...
        """
        Process documents through the full pipeline WITH RAG integration.
        
        Workflow:
        1. Parse documents (intelligent routing)
        2. Check embedding cache
        3. Create embeddings and store in Qdrant
        4. Classify documents (LLM-based)
        5. Extract content (LLM-based)
        6. Analyze document set
        7. Generate planning context
        
        Args:
            pdf_paths: List of paths to PDF files
            session_id: Session ID for Qdrant collection naming
            output_dir: Directory to save intermediate results
            force_reprocess: If True, bypass cache and re-process everything
        ...
        Returns:
            ...
            - extractions: List of ExtractedContent
            - analysis_report: DocumentAnalysisReport
            - parsed_documents: List of ParsedDocument
            - qdrant_manager: QdrantManager instance
            - qdrant_stats: Qdrant ingestion statistics
            - embedding_cache_stats: Cache hit/miss stats
            - cache_stats: Pipeline cache statistics (if caching enabled)
        """
        
        if self.verbose:
            self.console.rule("[bold blue]📚 Document Intelligence Pipeline with RAG[/bold blue]")
            self.console.print(f"[cyan]Processing {len(pdf_paths)} documents...[/cyan]\n")
        ...
        if self.verbose:
            self.console.rule("[bold yellow]🔐 Step 1: Initialize Embedding Cache[/bold yellow]")
        
        cache_manager = EmbeddingCacheManager(cache_dir="data/embedding_cache")
        cache_hits = 0
        cache_misses = 0
        
        if self.verbose:
            cache_stats = cache_manager.get_cache_stats()
            self.console.print(f"[green]Cache initialized: {cache_stats['total_documents_cached']} documents cached[/green]\n")
        
        # STEP 2: Intelligent Parsing with cache checking
        from src.core.intelligent_document_parser import IntelligentDocumentParser
        
        if self.verbose:
            self.console.rule("[bold yellow]🔍 Step 2: Intelligent Document Parsing[/bold yellow]")
        
        parser = IntelligentDocumentParser(
            session_id=session_id,
            output_base_dir="data/parsed_documents",
        ...
        for pdf_path in pdf_paths:
            file_hash = cache_manager.calculate_file_hash(pdf_path)
            
            if not force_reprocess:
                cached_info = cache_manager.check_cache(file_hash)
                
                if cached_info:
                    # Cache hit - document already processed
                    cache_hits += 1
                    cached_documents_info.append({
                        "file_hash": file_hash,
                    ...
                    if self.verbose:
                        self.console.print(
                            f"  [green]✓ Cache HIT:[/green] {Path(pdf_path).name} "
                            f"(reused {cached_info['times_reused']} times)"
                        )
                    
                    # Update session usage
                    cache_manager.update_session_usage(file_hash, session_id)
                    continue
            
            # Cache miss or force reprocess - parse document
            cache_misses += 1
            
            if self.verbose:
                self.console.print(f"  [yellow]Cache MISS:[/yellow] {Path(pdf_path).name} - parsing...")
        
        # Parse documents not in cache
        if cache_misses > 0:
            docs_to_parse = [
                pdf_path for pdf_path in pdf_paths
                if not any(cached["pdf_path"] == pdf_path for cached in cached_documents_info)
            ]
            parsed_documents = parser.parse_batch(docs_to_parse)
        
        if self.verbose:
            self.console.print(f"\n[green]✓ Parsing complete: {len(parsed_documents)} new, {cache_hits} cached[/green]\n")
        
        # STEP 3: Create Embeddings and Store in Qdrant
        from src.core.qdrant_manager import QdrantManager
        
        if self.verbose:
            self.console.rule("[bold yellow]🧠 Step 3: Creating Embeddings & Storing in Qdrant[/bold yellow]")
        
        collection_name = f"pm_agent_{session_id[:8]}"
        ...
        # Ingest newly parsed documents
        qdrant_stats = {"chunks_created": 0, "chunks_added": 0, "documents_processed": 0}
        
        if parsed_documents:
            qdrant_stats = qdrant_manager.ingest_documents(parsed_documents)
            
            # Add to cache
            for doc in parsed_documents:
                file_hash = cache_manager.calculate_file_hash(doc.file_path)
                cache_manager.add_to_cache(
                    ...
                    metadata={
                        ...
                        "chunk_count": qdrant_stats["chunks_created"] // len(parsed_documents),
                        "embedding_model": "text-embedding-3-large",
                        "sessions_used_in": [session_id]
                    }
                )
        
        # Copy cached embeddings to session collection
        for cached_doc in cached_documents_info:
            copy_result = qdrant_manager.copy_cached_embeddings(
        ...
        if self.verbose:
            ...
            self.console.print(f"  - Cached chunks: {sum(c.get('points_copied', 0) for c in [copy_result])}")
            self.console.print(f"  - Total chunks: {qdrant_stats.get('chunks_added', 0)}")
            self.console.print()
        
        # STEP 4-6: Continue with existing classification/extraction/analysis pipeline
        if self.verbose:
            self.console.rule("[bold yellow]📋 Step 4-6: Classification, Extraction & Analysis[/bold yellow]")
        ...
        # Get cache stats
        embedding_cache_stats = cache_manager.get_cache_stats()
        embedding_cache_stats["session_cache_hits"] = cache_hits
        embedding_cache_stats["session_cache_misses"] = cache_misses
        ...
        result = {
            ...
            "parsing_log_path": str(parser.metadata_dir / "parsing_log.json")
        }
