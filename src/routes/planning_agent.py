...
async def upload_documents(
    ...
    try:
        ...
        try:
            ...
            pipeline_result = pipeline.process_documents_with_rag(
            ...
            session.qdrant_manager = pipeline_result["qdrant_manager"]
            session.qdrant_collection_name = pipeline_result["qdrant_collection_name"]
            session.parsed_documents = pipeline_result["parsed_documents"]
            session.embedding_cache_stats = pipeline_result["embedding_cache_stats"]
            session.parsing_log_path = pipeline_result["parsing_log_path"]
            
            qdrant_stats = pipeline_result["qdrant_stats"]
            cache_stats = pipeline_result["embedding_cache_stats"]
            
            print(f"RAG processing complete:")
            print(f"  - Parsed: {len(pipeline_result['parsed_documents'])} documents")
            print(f"  - Chunks: {qdrant_stats.get('chunks_added', 0)}")
            print(f"  - Cache hits: {cache_stats.get('session_cache_hits', 0)}")
            print(f"  - Cache misses: {cache_stats.get('session_cache_misses', 0)}")
            
            # Enhanced message with RAG stats
            message = (
                f"Processed {len(uploaded_files)} files with RAG pipeline. "
                f"Created {qdrant_stats.get('chunks_added', 0)} vector embeddings. "
                f"Cache hits: {cache_stats.get('session_cache_hits', 0)}/{len(uploaded_files)}. "
                "Use this session_id for feasibility check and plan generation."
            )
        except Exception as e:
            print(f"Warning: RAG processing failed: {e}")
            print("Continuing with basic session - RAG features unavailable")
            message = (
                f"Files uploaded successfully but RAG processing failed: {str(e)}. "
                "Basic features available. Use this session_id for processing."
            )
...
async def generate_embeddings(request: GenerateEmbeddingsRequest):
    ...
    try:
        ...
        print(f"  - Collection: {pipeline_result['qdrant_collection_name']}")
        print(f"  - Chunks: {qdrant_stats.get('chunks_added', 0)}")
        print(f"  - Cache hits: {cache_stats.get('session_cache_hits', 0)}")
        
        return GenerateEmbeddingsResponse(
            session_id=request.session_id,
            collection_name=pipeline_result["qdrant_collection_name"],
            chunks_stored=qdrant_stats.get("chunks_added", 0),
            cache_hits=cache_stats.get("session_cache_hits", 0),
            cache_misses=cache_stats.get("session_cache_misses", 0),
            processing_time=execution_time,
            parsing_log=pipeline_result["parsing_log_path"]
        )
...
async def check_feasibility(request: FeasibilityRequest):
    ...
    try:
        ...
        from src.app.feasibility_agent import extract_text_from_pdfs, generate_feasibility_questions, save_development_context_to_json
